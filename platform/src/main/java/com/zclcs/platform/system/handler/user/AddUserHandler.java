 ...
 * @author zclcs
 */
public class AddUserHandler extends BasePermissionHandler {

    private final Logger log = LoggerFactory.getLogger(this.getClass());

    private final UserService userService;

    public AddUserHandler(PermissionProvider permissionProvider, UserService userService) {
    ...
    public void doNext(RoutingContext ctx) {
        ...
        userService.createUser(userAo).onComplete(user -> {
            RoutingContextUtil.success(ctx, user);
        }, error -> {
            log.error("新增用户失败", error);
            RoutingContextUtil.error(ctx, "新增用户失败");
        });
    }
